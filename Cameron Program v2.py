...
def guess_add(guess,blanks):#Determains index position of guess in secret word and inserts it into blanks. this however creates an extra element which needs to be deleted.
...
for i in range(force_number()):#Call function force_number to ask user how many games they want, use a for in range loop to run the game that many times
    ...
    secret_word = list(secret_word)#Converts the secret word to a list as strings are immutable
    ...
    while repeat_guess == True:#This loop checks the user input to make sure that it is a letter present in the word. This means that the guess_check function does'nt need to check if the letter is in the word and instead add the letter to blanks
        guess = force_letter()
        if guess in guessed_list:
            print("You have already guessed that")
        elif guess not in secret_word:
            guessed_list.append(guess)#append the guessed letter to the guessed_list to ensure that it isn't guessed again by the user
            lives -= 1
